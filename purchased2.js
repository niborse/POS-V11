...
function saveToInventory() {
    ...
    purchaseItems.forEach(async purchasedItem => {
        const formattedItem = {
            itemName: purchasedItem.class, // Assuming the class represents the item name
        ...
        const response = await fetch('https://mangodb-b3ni.onrender.com/purchased', {
            ...
            body: JSON.stringify(formattedItem) // Sending the formatted item
        });

        if (response.ok) {
            console.log('Item saved to inventory:', formattedItem);
        } else {
            console.error('Error saving item to inventory:', response.statusText);
        }
    });
